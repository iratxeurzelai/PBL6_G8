...
def leerRecetas():
    with open ("Recetas_procesadas.csv", encoding="utf8") as csv_file:
        ...
        for row in csv_reader:
                    receta = Receta(row['title'], row['calories'],row['fat'], row['protein'], row['sodium'], row['huevos'], row['frutas'], row['marisco'], row['lacteos'], row['frutos secos'], row['verduras'], row['legumbres'])
...
class MacroConstraint(Constraint[str, Receta]):
    ...
    def satisfied(self, assignment: Dict[str, Receta]) -> bool:
        ...
        #proteinas
        maxProteinas=130
        if (float(assignment[self.diaAnteriorP].protein)+float(assignment[self.diaAnteriorS].protein)) in range (20, 50): #20 40
            minProteinas = 60 #40
        else:
            minProteinas=20 #20
            if float(assignment[self.diaAnteriorP].protein) in range (100, 130): #100 130
                maxProteinas = 65 #90
            else:
        ...
        #Fat
        maxFat=180
        if (float(assignment[self.diaAnteriorP].fat)+float(assignment[self.diaAnteriorS].fat)) in range (10, 300): #100 120
            minFat = 20 #140
        else:
            minFat=10 #100
            if float(assignment[self.diaAnteriorP].fat) in range (7, 130): #160 180
                maxFat = 180 #150
            else:
                maxFat=180 #180
